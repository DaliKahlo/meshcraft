...
void CurveSmoother::sample_as_cbspline_interp(int degree, int dense,
                int num_points, double coord[], std::vector<CurvePoint>* splinepoints) {
  int l, i;
  int numResPoints;
  ...
  if(num_points > cs_temp_buf_size_-1) {
...
//  l = num_points - degree + 2;
  ...
  if (!(knots = create_centripetal_knotsequence(xp_, yp_, l, degree))) {
    printf("fail to create knot vector.");
    return;
  }
