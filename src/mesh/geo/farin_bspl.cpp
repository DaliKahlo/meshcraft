...
double deboor(int degree,double coeff[],double knot[],double u, int i)
{
	int k,j;
	double t1,t2;
	double coeffa[1024];  /* might need adjustment! */
...
void bspl_to_points(int degree, int l, double coeff_x[], double knot[], int dense,
		double points_x[], int *point_num)
{
	int i,ii,l2;
	double u;
...
void bspl_evaluate(int degree, int l, double coeff_x[], double knot[], double u, double *points_x)
{
	int l2=l+degree-1;
	*points_x=0;
	for (int i= +degree-1; i<l+degree-1; i++)
	{
	  if(knot[i+1]>knot[i])
		  if( knot[i] <= u && u < knot[i+1]) {
			*points_x =deboor(degree,coeff_x,knot,u,i);
			return;
		  }
	}
    return;
...
//	int index[2],numpts;
//	double points_x[1000],points_y[1000];
//	double mmbox[4], del_x, del_y;
//	double bspl_x[200],bspl_y[200],knot[200];
//  	double dum,x,y,xx,yy,tight,scale_x,scale_y;
//	/* ----------------------------- */
//
