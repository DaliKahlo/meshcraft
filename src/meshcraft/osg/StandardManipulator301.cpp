//$c2   XRL 07/09/2012 changed performMovement.
//$c1   XRL 07/09/2012 Copied from osg v3.0.1 and renamed.
//=========================================================================
//
 ...
 *
 * StandardManipulator2 code Copyright (C) 2010 PCJohn (Jan Peciva)
...
#include "StandardManipulator2.h"
...
int StandardManipulator2::numRelativeFlagsAllocated = 0;

int StandardManipulator2::allocateRelativeFlag()
{
...
/// Constructor.
StandardManipulator2::StandardManipulator2( int flags )
    : inherited(),
      _thrown( false ),
      _allowThrow( true ),
      ...
      _flags( flags ),
      _relativeFlags( 0 ),
	  __currentOperation( 0 )
{
}


/// Constructor.
StandardManipulator2::StandardManipulator2( const StandardManipulator2& uim, const CopyOp& copyOp )
...
void StandardManipulator2::setNode( Node* node )
{
    _node = node;
...
const Node* StandardManipulator2::getNode() const
{
    return _node.get();
}
...
/** Return node if attached.*/
Node* StandardManipulator2::getNode()
{
    return _node.get();
}
...
void StandardManipulator2::setVerticalAxisFixed( bool value )
{
...
void StandardManipulator2::setAnimationTime( const double t )
{
    if( t <= 0. )
    {
        finishAnimation();
...
double StandardManipulator2::getAnimationTime() const
{
    if( _animationData )
...
bool StandardManipulator2::isAnimating() const
{
    if( _animationData )
...
void StandardManipulator2::finishAnimation()
{
    if( !isAnimating() )
        return;
...
/** Move the camera to the default position.
    ...
    StandardManipulator2 implementation only updates its internal structures and
    recomputes its home position if autoComputeHomePosition is set.
    Descendant classes are expected to update camera position.*/
void StandardManipulator2::home( double /*currentTime*/ )
{
    if( getAutoComputeHomePosition() )
        computeHomePosition( NULL, ( _flags & COMPUTE_HOME_USING_BBOX ) != 0 );
...
/** Move the camera to the default position.
    ...
    StandardManipulator2 implementation only updates its internal data.
...
void StandardManipulator2::home( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    if( getAutoComputeHomePosition() )
    {
...
void StandardManipulator2::init( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    flushMouseEventStack();
...
bool StandardManipulator2::handle( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::handleFrame( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::handleResize( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    init( ea, us );
    us.requestRedraw();
...
bool StandardManipulator2::handleMouseMove( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    return false;
}
...
bool StandardManipulator2::handleMouseDrag( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    addMouseEvent( ea );
...
bool StandardManipulator2::handleMousePush( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    flushMouseEventStack();
    addMouseEvent( ea );
...
bool StandardManipulator2::handleMouseRelease( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::handleKeyDown( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::handleKeyUp( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    return false;
}
...
bool StandardManipulator2::handleMouseWheel( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
    return false;
}
...
void StandardManipulator2::getUsage( ApplicationUsage& usage ) const
{
...
bool StandardManipulator2::performMovement()
{
	...
	if( __currentOperation == 0 ) {
		// follow the OSG behavior
		...
		if( buttonMask == GUIEventAdapter::LEFT_MOUSE_BUTTON )
		{
			if( __currentOperation == 1 ) 
			{
				return performMovementLeftMouseButton( eventTimeDelta, dx, dy );
			}
			else if( __currentOperation == 2 )
			{
				return performMovementRightMouseButton( eventTimeDelta, dx, dy );
			}
			else if( __currentOperation == 3 )
			{
				return performMovementMiddleMouseButton( eventTimeDelta, dx, dy );
			}
		}
...
bool StandardManipulator2::performMovementLeftMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    return false;
}
...
bool StandardManipulator2::performMovementMiddleMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    return false;
}
...
bool StandardManipulator2::performMovementRightMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    return false;
}
...
bool StandardManipulator2::handleMouseDeltaMovement( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::performMouseDeltaMovement( const float dx, const float dy )
{
   return false;
}
...
bool StandardManipulator2::performAnimationMovement( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
void StandardManipulator2::applyAnimationStep( const double currentProgress, const double prevProgress )
{
}


/// Centers mouse pointer
void StandardManipulator2::centerMousePointer( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
void StandardManipulator2::addMouseEvent( const GUIEventAdapter& ea )
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
}
...
void StandardManipulator2::flushMouseEventStack()
{
    _ga_t1 = NULL;
    _ga_t0 = NULL;
}
...
bool StandardManipulator2::isMouseMoving() const
{
...
void StandardManipulator2::setAllowThrow( bool allowThrow )
{
...
float StandardManipulator2::getThrowScale( const double eventTimeDelta ) const
{
...
void StandardManipulator2::rotateYawPitch( Quat& rotation, const double yaw, const double pitch,
                                           const Vec3d& localUp )
{
...
void StandardManipulator2::fixVerticalAxis( Vec3d& eye, Quat& rotation, bool disallowFlipOver )
{
 ...
 *  manipulator is rotated by 180 degrees. More precisely, roll rotation component is changed by 180 degrees,
 *  making pitch once again between -90..+90 degrees limits.*/
void StandardManipulator2::fixVerticalAxis( Quat& rotation, const Vec3d& localUp, bool disallowFlipOver )
{
 ...
 *  making pitch once again between -90..+90 degrees limits.*/
void StandardManipulator2::fixVerticalAxis( const osg::Vec3d& forward, const osg::Vec3d& up, osg::Vec3d& newUp,
                                           const osg::Vec3d& localUp, bool disallowFlipOver )
{
    ...
    if( updatedUp.normalize() >= 0. )
       ...
       // return original up
       OSG_WARN << "StandardManipulator2::fixVerticalAxis warning: Can not update vertical axis." << std::endl;
       newUp = up;
...
/** The method sends a ray into the scene and the point of the closest intersection
    ...
    towards the center.*/
bool StandardManipulator2::setCenterByMousePointerIntersection( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
bool StandardManipulator2::startAnimationByMousePointerIntersection(
...
StandardManipulator2::AnimationData::AnimationData()
    :_isAnimating( false )
{
}


void StandardManipulator2::AnimationData::start( const double startTime )
{
    _isAnimating = true;
