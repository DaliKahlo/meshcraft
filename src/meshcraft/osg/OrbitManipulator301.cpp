...
int OrbitManipulator2::_minimumDistanceFlagIndex = allocateRelativeFlag();


/// Constructor.
OrbitManipulator2::OrbitManipulator2( int flags )
   : inherited( flags ),
     _distance( 1. ),
     _trackballSize( 0.8 )
{
...
/// Constructor.
OrbitManipulator2::OrbitManipulator2( const OrbitManipulator2& om, const CopyOp& copyOp )
   : inherited( om, copyOp ),
     _center( om._center ),
...
void OrbitManipulator2::setByMatrix( const osg::Matrixd& matrix )
{
...
/** Set the position of the manipulator using a 4x4 matrix.*/
void OrbitManipulator2::setByInverseMatrix( const osg::Matrixd& matrix )
{
...
osg::Matrixd OrbitManipulator2::getMatrix() const
{
...
osg::Matrixd OrbitManipulator2::getInverseMatrix() const
{
...
// doc in parent
void OrbitManipulator2::setTransformation( const osg::Vec3d& eye, const osg::Quat& rotation )
{
...
// doc in parent
void OrbitManipulator2::getTransformation( osg::Vec3d& eye, osg::Quat& rotation ) const
{
...
// doc in parent
void OrbitManipulator2::setTransformation( const osg::Vec3d& eye, const osg::Vec3d& center, const osg::Vec3d& up )
{
    Vec3d lv( center - eye );
...
// doc in parent
void OrbitManipulator2::getTransformation( osg::Vec3d& eye, osg::Vec3d& center, osg::Vec3d& up ) const
{
    center = _center;
    eye = _center + _rotation * osg::Vec3d( 0., 0., _distance );
...
void OrbitManipulator2::setHeading( double azimuth ) 
{
...
double OrbitManipulator2::getHeading() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
...
void OrbitManipulator2::setElevation( double elevation )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
    Vec3d localRight = getSideVector( coordinateFrame );
...
double OrbitManipulator2::getElevation() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
    localUp.normalize();
...
// doc in parent
bool OrbitManipulator2::handleMouseWheel( const GUIEventAdapter& ea, GUIActionAdapter& us )
{
...
//bool OrbitManipulator2::performMovement()
//{
...
// doc in parent
bool OrbitManipulator2::performMovementLeftMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    // rotate camera
    if( getVerticalAxisFixed() )
...
// doc in parent
bool OrbitManipulator2::performMovementMiddleMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    // pan model
...
// doc in parent
bool OrbitManipulator2::performMovementRightMouseButton( const double eventTimeDelta, const double dx, const double dy )
{
    // zoom model
...
bool OrbitManipulator2::performMouseDeltaMovement( const float dx, const float dy )
{
    // rotate camera
    if( getVerticalAxisFixed() )
        rotateWithFixedVertical( dx, dy );
...
void OrbitManipulator2::applyAnimationStep( const double currentProgress, const double prevProgress )
{
...
bool OrbitManipulator2::startAnimationByMousePointerIntersection(
...
void OrbitManipulator2::OrbitAnimationData::start( const osg::Vec3d& movement, const double startTime )
{
...
void OrbitManipulator2::rotateTrackball( const float px0, const float py0,
...
void OrbitManipulator2::rotateWithFixedVertical( const float dx, const float dy )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    Vec3d localUp = getUpVector( coordinateFrame );
...
void OrbitManipulator2::rotateWithFixedVertical( const float dx, const float dy, const Vec3f& up )
{
...
void OrbitManipulator2::panModel( const float dx, const float dy, const float dz )
{
...
/** Changes the distance of camera to the focal center.
    ...
    \sa OrbitManipulator2::setMinimumDistance
 */
void OrbitManipulator2::zoomModel( const float dy, bool pushForwardIfNeeded )
{
    // scale
    float scale = 1.0f + dy;
 ...
 * (-1.0 ... 1.0)
 */
void OrbitManipulator2::trackball( osg::Vec3d& axis, float& angle, float p1x, float p1y, float p2x, float p2y )
{
    /*
 ...
 */
float OrbitManipulator2::tb_project_to_sphere( float r, float x, float y )
{
    float d, t, z;
...
osgUtil::SceneView::FusionDistanceMode OrbitManipulator2::getFusionDistanceMode() const
{
    return osgUtil::SceneView::USE_FUSION_DISTANCE_VALUE;
}

/** Get the FusionDistanceValue. Used by SceneView for setting up stereo convergence.*/
float OrbitManipulator2::getFusionDistanceValue() const
{
    return _distance;
}
...
void OrbitManipulator2::setCenter( const Vec3d& center )
{
    _center = center;
}
...
const Vec3d& OrbitManipulator2::getCenter() const
{
    return _center;
}
...
void OrbitManipulator2::setRotation( const Quat& rotation )
{
    _rotation = rotation;
}
...
const Quat& OrbitManipulator2::getRotation() const
{
    return _rotation;
}
...
void OrbitManipulator2::setDistance( double distance )
{
    _distance = distance;
}
...
double OrbitManipulator2::getDistance() const
{
    return _distance;
}
...
void OrbitManipulator2::setTrackballSize( const double& size )
{
    /*
...
void OrbitManipulator2::setWheelZoomFactor( double wheelZoomFactor )
{
...
void OrbitManipulator2::setMinimumDistance( const double& minimumDistance, bool relativeToModelSize )
{
...
/** Get the minimum distance of the eye point from the center
    before the center is pushed forward.*/
double OrbitManipulator2::getMinimumDistance( bool *relativeToModelSize ) const
{
